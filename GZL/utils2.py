...
from pygsp.graphs import fourier, difference  # prevent circular import in Python < 3.5
import sys
...
class Graph(fourier.GraphFourier, difference.GraphDifference):
    ...
    * Initialize attributes for derived classes.
    ...
    def __init__(self, W, N, M, gtype='unknown', lap_type='combinatorial',
        ...
        data = np.ones(M)
        # sys.exit()
        W = sparse.csr_matrix((data, (W[0], W[1])), shape=(N, N))
        W.eliminate_zeros()
    ...
    def subgraph(self, ind):
        ...
        return gsp.graphs.Graph(sub_W, gtype="sub-{}".format(self.gtype))
